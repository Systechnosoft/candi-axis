...
async function diagnose() {
  ...
  try {
    // Which database?
    const dbName = await client.query(
    ...
    const schemas = await client.query(`
      SELECT table_schema, table_name, 
        (SELECT COUNT(*) FROM information_schema.columns WHERE table_schema = t.table_schema AND table_name = 'ca_users') as col_count
      FROM information_schema.tables t
      WHERE table_name = 'ca_users'
      ORDER BY table_schema
    `);
    console.log('Users tables found in schemas:', schemas.rows);

    // Check how many rows in the users table in public schema
    const count = await client.query('SELECT COUNT(*) FROM public.ca_users');
    console.log('public.users row count:', count.rows[0].count);

    // Show all users
    const users = await client.query(
      'SELECT id, email, email_normalized, is_active, status, is_deleted FROM public.ca_users LIMIT 10',
    );
