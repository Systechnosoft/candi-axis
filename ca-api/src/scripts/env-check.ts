...
if (!allOk) {
  console.error(
    '[env-check] Environment validation FAILED. Check your .env file.',
  );
  process.exit(1);
}
...
if (bootstrapEmail && !serviceRoleKey) {
  console.warn(
    '[env-check] WARNING: BOOTSTRAP_ADMIN_EMAIL is set but SUPABASE_SERVICE_ROLE_KEY is missing. Bootstrap admin will be skipped.',
  );
}
