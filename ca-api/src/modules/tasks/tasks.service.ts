...
export class TasksService {
  ...
  async findAll(userId: string) {
    ...
    let query = `
      SELECT t.*, c.full_name as candidate_name, jd.title as job_title, jd.code as jd_code,
       ...
       JOIN job_descriptions jd ON t.jd_id = jd.id
       LEFT JOIN job_postings jp ON t.jobposting_id = jp.id
    `;
  ...
  async findOne(taskId: string, userId: string) {
    ...
    if (isAdmin) {
      ...
      const hasHrAccess = roles.includes('hr_recruiter') && (isAssignee || isHr);
      const hasHMAccess = roles.includes('hiring_manager') && (isAssignee || isOwner);
    ...
    const formattedName = task.candidate_name ? `${task.name}-${task.candidate_name}(#${task.task_id})` : task.name;
    const taskMetadata = {
      ...task,
      name: formattedName,
    };
    ...
    if (task.status === 'new' || task.status === 'read') {
      await this.pool.query(
        `UPDATE tasks SET status = 'open', updated_at = now() WHERE task_id = $1`,
        [id]
      );
    ...
    const isOnlyInterviewer = roles.includes('interviewer') &&
    ...
    const [
      ...
      this.pool.query(`SELECT * FROM candidates WHERE id = $1 AND is_deleted = false`, [candidateId]),
      this.pool.query(`SELECT * FROM candidate_educations WHERE candidate_id = $1 AND is_deleted = false ORDER BY sort_order ASC, start_year DESC`, [candidateId]),
      this.pool.query(`SELECT * FROM candidate_employments WHERE candidate_id = $1 AND is_deleted = false ORDER BY sort_order ASC, start_date DESC`, [candidateId]),
      this.pool.query(`SELECT * FROM candidate_certifications WHERE candidate_id = $1 AND is_deleted = false ORDER BY sort_order ASC`, [candidateId]),
      this.pool.query(`SELECT * FROM candidate_social_links WHERE candidate_id = $1 AND is_deleted = false ORDER BY sort_order ASC`, [candidateId]),
      this.pool.query(`SELECT * FROM candidate_projects WHERE candidate_id = $1 AND is_deleted = false ORDER BY sort_order ASC`, [candidateId]),
      this.pool.query(
      ...
      this.pool.query(`SELECT * FROM job_descriptions WHERE id = $1`, [jdId]),
      this.pool.query(`SELECT rating FROM job_candidate_matches WHERE job_id = $1 AND candidate_id = $2 AND deleted_at IS NULL LIMIT 1`, [jdId, candidateId]),
      this.pool.query(
  ...
  async submitFeedback(taskId: string, userId: string, action: 'approve' | 'reject', reason: string) {
    const id = parseInt(taskId, 10);
    if (isNaN(id)) {
      throw new BadRequestException('Invalid task ID format');
    }
    ...
    if (task.submitted_on !== null || !task.is_active) {
      return {
        success: false,
        message: 'This screening review has already been completed by another interviewer.',
      };
    }

    // Check if user is an assignee
    if (!task.assignee.includes(userId)) {
      throw new BadRequestException('You are not authorized to submit feedback for this task.');
    }
    ...
    try {
      ...
      await this.auditService.log({
        ...
        afterJson: { is_active: false, submitted_on: new Date(), feedback_action: action, feedback_reason: reason },
      ...
      if (action === 'approve') {
        ...
        try {
          await this.stageService.updateStage(task.application_id, userId, 'interviewing', 'Screening approved by interviewer');
        } catch (err: any) {
          // If updateStage fails, we log it, but the task feedback submission is already saved.
          this.pool.query(
            `UPDATE tasks SET error_message = $1 WHERE task_id = $2`,
