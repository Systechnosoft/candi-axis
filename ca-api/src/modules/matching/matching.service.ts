...
export class MatchingService {
   ...
   */
  async getStoredMatches(jobId: string): Promise<Array<{ candidateId: string; rating: number; createdAt: Date }>> {
    const query = `
      SELECT 
        jcm.candidate_id as "candidateId", 
        jcm.rating, 
        jcm.created_at as "createdAt" 
      FROM job_candidate_matches jcm
   ...
   */
  async storeMatches(orgId: string, matches: Array<{ jobId: string; candidateId: string; rating: number }>): Promise<void> {
    ...
    const query = `
      INSERT INTO job_candidate_matches (org_id, job_id, candidate_id, rating)
  ...
  async findMatches(jobId: string): Promise<any> {
    ...
    const processedCandidates = await this.getAlreadyProcessedCandidateIds(jobId);

    const allCandidatesRes = await this.pool.query(
      `SELECT id FROM candidates WHERE status = 'active' AND is_deleted = false`
    );
    const allCandidates = allCandidatesRes.rows;

    const onlyNew = allCandidates.filter(c => !processedCandidates.includes(c.id));
    ...
    if (newCandidateIds.length > 0) {
      ...
      `;
      const res = await this.pool.query(matchingQuery, [jobId, newCandidateIds]);
    ...
    this.logger.log(`Job-Candidate Matching for Job ID ${jobId} finished: ` +
      `stored matches found = ${storedMatches.length}, ` +
      `new candidates processed = ${newCandidateIds.length}, ` +
      `time taken = ${timeTaken}ms`
    );

    const formattedMatches = storedMatches.map(m => ({
