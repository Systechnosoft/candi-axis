...
export class AuthService {
   ...
   */
  async resolveAtsUser(supabaseUserId: string, email: string): Promise<{ atsUserId: string; email: string }> {
    this.logger.debug(`Resolving ATS user for Supabase ID: ${supabaseUserId}, Email: ${email}`);
    ...
    const byId = await this.pool.query(
      `SELECT id, email, is_active, status, is_deleted
       FROM users WHERE supabase_auth_user_id = $1 LIMIT 1`,
      [supabaseUserId],
    );
    ...
    if (!user) {
      ...
      const byEmail = await this.pool.query(
        `SELECT id, email, is_active, status, is_deleted
         FROM users WHERE email_normalized = $1 AND supabase_auth_user_id IS NULL LIMIT 1`,
        [normalized],
      );
      user = byEmail.rows[0];
      ...
      if (user) {
        this.logger.debug(`Found user by normalized email: ${user.id}. Linking Supabase ID...`);
        await this.pool.query(
          `UPDATE users SET supabase_auth_user_id = $1 WHERE id = $2`,
          [supabaseUserId, user.id],
        );
        this.logger.log(`Linked supabase_auth_user_id to existing ATS user: ${email}`);

        // Audit log the mapping
        await this.pool.query(
          `INSERT INTO audit_logs (entity_type, entity_id, action, before_json, after_json, reason_context)
        ...
        this.logger.debug(`Fallback failed. user not found by email or already linked to another ID.`);
      }
    }

    if (!user) {
      this.logger.warn(`No ATS user mapping found for Supabase user: ${supabaseUserId} (${email})`);
    ...
    if (user.is_deleted || !user.is_active || user.status !== 'active') {
      this.logger.warn(`ATS user is inactive/deleted: ${email} (status: ${user.status}, is_active: ${user.is_active}, is_deleted: ${user.is_deleted})`);
